        ...
        <tbody>
            @foreach($attempts as $attempt)
                ...
                <tr>
                    ...
                    <td>
                        <div style="background-color: {{ $bgColor }}; padding: 5px 10px; border-radius: 5px; text-align: center; font-weight: bold;">
                            {{ $attempt->score }} / {{ $attempt->total_questions }}
                        </div>
                    </td>
                    <td>{{ $attempt->created_at->format('M d, Y H:i') }}</td>
                    <td>
                        <a href="{{ route('student.quizzes.results', [$attempt->quiz, $attempt]) }}" class="btn btn-sm btn-info">
                            View Feedback
                        </a>
                    </td>
                </tr>
