...
@section('content')
<form method="POST" action="{{ route('quizzes.questions.update', [$quiz, $question]) }}" id="question-form">
    @csrf
    @method('PUT')
    <div class="container">
            ...
            @foreach ($question->options as $i => $option)
                <div class="mb-2">
                    <input type="text" name="options[{{ $i }}][option_text]" class="form-control d-inline-block w-75" 
                        placeholder="Option {{ $i + 1 }}" value="{{ old('options.'.$i.'.option_text', $option->option_text) }}" required>
